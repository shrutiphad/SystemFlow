const CELLS = (summary) => [
  { label: 'TOTAL TASKS', value: summary.total, tone: 'gradient-text' },
...
export default function DashboardStats({ summary }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-line bg-line shadow-card dark:border-line-dark dark:bg-line-dark sm:grid-cols-5">
      {CELLS(summary).map((cell) => (
        <div key={cell.label} className="bg-surface p-4 transition-colors hover:bg-surface2 dark:bg-surface-dark dark:hover:bg-surface2-dark">
